    ...
    <Layout>
      <section className="min-h-screen flex flex-col lg:flex-row pt-20 transition-colors duration-300">
        ...
        <div className="lg:w-[45%] lg:h-[calc(100vh-80px)] lg:sticky lg:top-20 hidden lg:block overflow-hidden relative">
          ...
          <div className="absolute bottom-12 ltr:left-12 rtl:right-12 ltr:right-12 rtl:left-12 text-white z-[3]">
        ...
        <div className="lg:w-[55%] p-8 lg:p-16 bg-bg transition-colors duration-300">
          <div className="max-w-xl mx-auto">
            <div>
              <h1 className="text-4xl font-display mb-2 text-text leading-tight">{t('book.title')}</h1>
              <p className="text-text-muted font-body mb-12">
                {t('book.sub')}
              </p>

              <div className="mt-12 pt-12 border-t border-gold/10">
                <a href="https://wa.me/90701506322" target="_blank" className="flex items-center justify-center gap-2 p-4 bg-[#25D366] text-white rounded-xl font-bold transition-transform hover:scale-105 max-w-xs mx-auto">
      ...
      <section className="py-32 px-4 bg-surface transition-colors duration-300 border-y border-border">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
            ...
            {[
              { id: 1, title: t('book.step1_title'), text: t('book.step1_text'), icon: <Send className="w-6 h-6 rtl:-scale-x-100" /> },
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            >
              ...
              <h3 className="text-2xl font-display font-bold text-text leading-tight">{card.title}</h3>
          ...
          <div className="lg:w-1/2 relative h-[500px] w-full rounded-[40px] overflow-hidden shadow-2xl">
            ...
            <div className="absolute inset-0 bg-gold/20 flex items-center justify-center">
              <div className="bg-white/90 backdrop-blur-xl p-8 rounded-3xl shadow-2xl text-center">
                <span className="text-gold font-bold uppercase tracking-widest rtl:tracking-normal text-xs mb-2 block">{t('book_page.pack_include')}</span>
                <h4 className="text-3xl font-display text-black">{t('book_page.pack_title')}</h4>
              </div>
            </div>
          </div>
          <div className="lg:w-1/2">
            <h2 className="text-5xl font-display mb-8 text-text leading-tight">{t('book_page.prep_title')}</h2>
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={i} className="flex gap-4">
                  ...
                  <div>
                    <h5 className="font-display font-bold text-xl text-text leading-tight">{item.title}</h5>
      ...
      <section className="py-32 px-4 bg-surface transition-colors duration-300 border-y border-border">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-5xl font-display text-center mb-20 text-text leading-tight">{t('book.faq_title')}</h2>
          <Accordion>
            {faqs.map((faq, i) => (
              <AccordionItem
                key={i}
